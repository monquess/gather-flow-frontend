...
const AttendeesListModal: React.FC<AttendeesListModalProps> = ({
	...
	const fetchAttendees = async (): Promise<AttendeesResponse> => {
		const { data } = await apiClient(`/events/${id}/attendees?page=${page}`)
		return data
	}
		...
		>
			...
			>
				{isLoading ? (
					<Center h={200}>
						<Loader />
						<Text ml="md">{t('attendeesList.loading')}</Text>
					</Center>
				) : data?.data.length === 0 ? (
					<Center h={200}>
						<Text c="dimmed">{t('attendeesList.noAttendees')}</Text>
					</Center>
				) : (
					<Stack gap="xs">
